...
export class AuditController {
    static async getLogs({ query }: { query: { page?: string; limit?: string; entityId?: string; search?: string; action?: string; entity?: string; startDate?: string; endDate?: string } }) {
        try {
            if (query.entityId) {
            ...
            return await AuditService.getLogs(page, limit, search, action, entity, startDate, endDate);
        } catch (error) {
            console.error(error);
